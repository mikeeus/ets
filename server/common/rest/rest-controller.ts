...
export class RestController {
  ...
  validateModel(response: Response, model: Validatable): void {
    const validatorErrors: Array<ValidatorError> = model.validate();
    return this.throwValidatorErrors(validatorErrors);
  }
  ...
  private throwValidatorErrors(validatorErrors: Array<ValidatorError>): any {
    ...
    return new ValidationFailureError(failures);
  }
}